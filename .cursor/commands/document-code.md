...
- **Create ASCII mockups** showing the UI as it actually renders
...
### ASCII Mockups (Extracted from Code)

Default State:
┌─────────────────────────────────────┐
│ [Component as it currently renders] │
└─────────────────────────────────────┘
...
- `.specs/features/{domain}/{feature}.feature.md` - Feature spec (with ASCII mockups)
...
### Mockup vs. Behavior Gaps

⚠️ Visual inconsistencies found:
- Mockup shows X but code renders Y
- Missing state: no visual for error condition
